...
# CHANGE: Define list of 6 ghost filenames
...
def create_nextbot_textures():
    """Loads all 6 ghost textures and returns a list of texture IDs."""
...
# CHANGE: Variable is now a list of textures
nextbot_textures = [] 
killer_tex_id = None # Stores the texture of the ghost that killed you
...
def add_new_ghost():
    ...
    # CHANGE: Assign a random texture index (0 to 5) to the ghost
    tex_idx = random.randint(0, len(nextbot_textures) - 1)
    
    ghosts.append({
        'x': gx, 
        'y': gy, 
        'color': (1.0, 1.0, 1.0),
        'tex_idx': tex_idx # Store which image this ghost uses
    })
...
def relocate_ghost(ghost_index):
    ...
    # Keep the same texture or randomize? Let's keep same for identity consistency
    # ghosts[ghost_index]['tex_idx'] = random.randint(0, len(nextbot_textures) - 1)
    total_hits += 1
...
def update(dt):
    ...
    for ghost in ghosts:
        ...
        if dist < 3.0:
            if not is_autopilot: 
                # CHANGE: Store which ghost texture killed you
                if len(nextbot_textures) > 0:
...
def draw_nextbots():
    ...
    for ghost in ghosts:
        # CHANGE: Bind the specific texture for this ghost
        idx = ghost['tex_idx']
...
def draw_jumpscare():
    ...
    # CHANGE: Draw the face of the ghost that killed you
    glEnable(GL_TEXTURE_2D)
    if killer_tex_id is not None:
        glBindTexture(GL_TEXTURE_2D, killer_tex_id)
    elif len(nextbot_textures) > 0:
        glBindTexture(GL_TEXTURE_2D, nextbot_textures[0]) # Fallback
...
def setup_lighting():
    ...
    if is_torch_on:
        ...
        # --- BRIGHTNESS LEVEL 5 (Balanced) ---
        # Beam Width: Standard tactical width
        glLightf(GL_LIGHT0, GL_SPOT_CUTOFF, 40.0) 
        # Focus: Standard soft edge (not too sharp, not too washed out)
        glLightf(GL_LIGHT0, GL_SPOT_EXPONENT, 8.0)
        
        # ATTENUATION (Reach): Balanced values
        glLightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, 0.1) 
        glLightf(GL_LIGHT0, GL_LINEAR_ATTENUATION, 0.02)     # 0.005 (Lvl 10) <-> 0.05 (Lvl 1)
        glLightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, 0.003)  # 0.0001 (Lvl 10) <-> 0.01 (Lvl 1)
        
        # INTENSITY: Standard White
        glLightfv(GL_LIGHT0, GL_DIFFUSE, [1.0, 1.0, 1.0, 1]) 
        glLightfv(GL_LIGHT0, GL_AMBIENT, [0.3, 0.3, 0.3, 1]) # Moderate ambient fill
    else:
        glDisable(GL_LIGHT0)
        glEnable(GL_LIGHTING)
        glEnable(GL_COLOR_MATERIAL)
...
def main():
    ...
    glutCreateWindow(b"Nextbot AI: [F] View, [T] Torch, [SPACE] Throw Ball at BLUE Walls")
    init()
